...
struct chunk {
    ...
    void setPad(const unsigned length = 1) { setInt(0, length); }
    ...
    private:
        ...
        void set_fcc(const fourcc &fcc, std::vector<unsigned char> &out) const {
            const unsigned tmp = fcc.getInt();
            for (int f = 0; f < 4; ++f) out.push_back((tmp >> (8 * f)) & 0xFF);
        }
